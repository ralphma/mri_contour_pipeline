...
class DataPoint(object):
    ...
    def as_pandas_frame(self):
        ...
        return pd.DataFrame(data={"dicom_image":[self.dicom_image], "contour_image":[self.contour_image]},
                            index=[self.dicom_path])
...
class Dataset(object):
    def __init__(self, shuffle_data, return_indefinitely,
                 dicom_file_glob=_DICOM_FILE_GLOB, contour_file_glob=_I_CONTOUR_GLOBS):
        self.data_points = []
        ...
        # The file glob used to find dicom files, if not set, using default "*.dcm"
        self.dicom_file_glob = dicom_file_glob

        # The file glob used to find contour files, if not set, using default of "i-contours/*.txt"
        self.contour_file_glob = contour_file_glob
    ...
    def parse_from_csv(self, link_csv, dicom_dir, contour_dir):
        ...
        for index, row in dicom_to_contour.iterrows():
            ...
            all_dicom_files = glob.glob(os.path.join(dicom_dir, patient_id, self.dicom_file_glob))
            all_contour_files = glob.glob(os.path.join(contour_dir, original_id, self.contour_file_glob))
    ...
    def as_panda_dataframe(self):
        """Return the dataset as a panda dataframe for easier analysis."""
        all_data_frames = [x.as_pandas_frame() for x in self.data_points]
    ...
    def __next__(self):
        ...
        if not self.data_points:
            raise StopIteration
        if self._count == len(self.data_points):
            if self.return_indefinitely:
                self._reset()
            else:
                raise StopIteration
        datapoint = self.data_points[self._count]
        self._count+=1
        return datapoint